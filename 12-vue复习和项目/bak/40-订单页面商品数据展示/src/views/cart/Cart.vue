          ...
          <div class="shopCartListCon" v-for="(goods,index) in shopCart" :key="goods.id">
            <div class="left">
              <a
                ...
                @click.stop="singeGooodsSelected(goods.id)"
              ></a>
            </div>
            <div class="center">
      ...
      <div class="tabBar">
        <div class="tabBarLeft">
          <a
            ...
            class="cartCheckBox"
            :checked="isSelected"
            @click.stop="selectAll(isSelected)"
          ></a>
        ...
        <div class="tabBarRight">
          <router-link :to="{path:'/confirmOrder'}" class="pay">去结算({{goodsCount}})</router-link>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    goodsCount() {
      // 商品的件数
      let selectedGoodsGount = 0;
      Object.values(this.shopCart).forEach((goods, index) => {
        if (goods.checked) {
          selectedGoodsGount += 1;
        }
      });
      return selectedGoodsGount;
    }
  },
  methods: {
    ...mapMutations([
    ...
    async removeOutCart(goodsId, goodsNum) {
      if (goodsNum > 1) {
        // 先调用接口，把服务器端购物车的数量减1，成功后，再操作本地
        let result = await changeCartNum(this.userInfo.token,goodsId,"reduce");
        if(result.success_code === 200){
          this.REMOVE_CART({ goodsId });  // 把vuex和localstrage中的商品的数量减1
        }else{
          // 表示服务器端购物车列表并没有成功减1
          Toast({
    ...
    // 单个商品的选中与取消选中
    async singeGooodsSelected(goodsId) {
      let result = await singerGoodsSelect(this.userInfo.token,goodsId)
      if(result.success_code === 200){
        // 操作一个mutations  因为你要改变vuex中的数据 必须通过mutation
