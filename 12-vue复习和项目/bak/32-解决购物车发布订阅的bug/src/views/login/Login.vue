        ...
        <div class="login-inner">
            ...
            <div class="login-header">
                <div class="login-logo">
                    <h1 style="font-size:20px;">京东买菜</h1>
                </div>
                <!--面板标题-->
            ...
            <div class="login-content">
                <form>
                    ...
                    <div :class="{current: loginMode}">
                        ...
                        <section class="login-hint">
                            温馨提示：未注册京东买菜的手机号，登录时将自动注册，且代表已同意
                    ...
                    <div :class="{current: !loginMode}">
                        <section>
                            ...
                            <section class="login-verification">
                                ...
                                <input else type="text" maxlength="20" placeholder="密码" autocomplete="off" v-model="pwd"/>
                ...
                <button class="login-back" @click.prevent="$router.go(-1)">返回</button>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        data(){
            return {
                ...
                user_name:null, // 用户名
                pwd:null, //密码
                pwdMode:true, // true表示密码是密文  false表示明文
                captcha:null, // 验证码 
            }
        },
        computed:{
        ...
        methods: {
            ...
            async login(){
                ...
                if(this.loginMode){ // 手机验证码登录
                    ...
                    // 发起请求  直接输入一个用户名  没有注册过  自动注册 
                    let result = await pwdLogin(this.user_name, this.pwd, this.captcha);
                    // console.log(result)
                    if(result.success_code === 200){
                        // 保存用户信息  请求接口获取数据是异步 为了更好追踪vuex中的状态 通过actions
                        // actions需要commit一个mutations
                        this.syncUserInfo(result.data) // 保存到了vuex和localstorage中
            ...
            // 5. 获取验证码
            getCaptcha(){
               let captchaEle = this.$refs.captcha; // 得到验证码的图片 
               // 给这个图片设置一个src属性  vue中设置属性 $set 
               // 这么写请求是发不出来的， 浏览器会走缓存 ， 由于接口地址一样
               // this.$set(captchaEle,"src","http://localhost:3000/web/xlmc/api/captcha")
               // 要想不走缓存 你需要保证地址不一样  如何保存地址不一样  传达一个时间  或来一个随机数
               // 目的：保存你每一次点击时，url是不一样的
