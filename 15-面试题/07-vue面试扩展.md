...
## 5. 使用组件标签上使用v-model
    1). v-model的本质
        <input v-model="name">
        相当于给input标签传递了一个动态的value属性(属性值为name)，并且绑定了一个input监听（原生的），在input监听中将最新的值（$event.target.value）赋值给name,这样data中的数据发生变化了，就会实时显示
        <input :value="name" @input="name = $event.target.value">
    2). 在自定义组件上使用v-model
        <MyInput v-model="name">
        <MyInput :value="name" @input="handleInput"></MyInput>
        相当于给MyInput组件对象标签传递了一个动态的value属性(属性值为name)，并且绑定了一个input监听（自定义）
        
        MyInput.vue
            props: ['value']
            <input :value='value' @input="$emit('input', $event.target.value)">
        然后将value传递给MyInput组件，在MyInput组件中给input标签绑定原生input监听,分发事件 再将最新的值赋值给name

## 6. vue的响应式原理
    1). 关注点有哪些?
        vue的数据绑定效果: 组件更新data数据后, 当前组件及相关的子组件都会更新相应的节点
        如何知道数据变化了?
        Object.defineProperty()给data属性添加 setter 监视数据变化
        通知哪些组件更新渲染?
        组件更新渲染是同步还是异步的?
        异步：数据变了并不代表界面变了，而是先去调用监视的回调函数，才异步更新界面，所以我们才用到了$nextTick()
    ...
    3). 一些细节
        ...
        data中所有层次属性都是响应式的
            data中的属性分为对象和数组
                当为对象时内部属性的响应式是通过set方法监视的，
                而为数组是其内部属性的响应式靠重写方法，只有调用方法。才可以改变某一个元素
                ，不可以通过下标去替换某一个元素
            当删除data中的某一个属性时,data也不会知道，因为煤油触发set方法调用，只有给data中的属性赋了一个新值才会调用set监视【set是用来监视属性值的变化的】
        ...
        关于<Root>组件标签: 
            ...
            只有new Vue({})，并且挂到页面上后才能产生根标签 <Root>
            <Root>对应的是vm
...
![](file:///C:/Users/Fei/Documents/My%20Knowledge/temp/3d32279e-d65b-46dc-8e1b-eee2764af93c/128/index_files/7.%20vue%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F1.png)
    ...
    3). 一些细节
		...
		destroyed():  实例销毁后调用, 数据绑定/事件监听器都没了, 但dom结构还在
